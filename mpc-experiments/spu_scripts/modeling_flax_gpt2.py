...
class FlaxGPT2Attention(nn.Module):
    ...
    def setup(self):
        ...
        if self.is_cross_attention:
            ...
            if USE_LORA:
                self.c_attn_A = FlaxConv1D(3 * RANK, dtype=self.dtype, use_bias=False)
                self.c_attn_B = FlaxConv1D(3 * self.embed_dim, dtype=self.dtype, use_bias=False)
            else:
                self.c_attn = FlaxConv1D(3 * self.embed_dim, dtype=self.dtype)
        if USE_LORA:
            self.c_proj_A = FlaxConv1D(RANK, dtype=self.dtype, use_bias=False)
            self.c_proj_B = FlaxConv1D(self.embed_dim, dtype=self.dtype, use_bias=False)
        else:
    ...
    def __call__(
        ...
        if not is_cross_attention:
            if USE_LORA:
                qkv_out_A = self.c_attn_A(hidden_states)
                qkv_out = self.c_attn_B(qkv_out_A)
            else:
        ...
        if USE_LORA:
            attn_output_A = self.c_proj_A(attn_output)
            attn_output = self.c_proj_B(attn_output_A)
        else:
...
class FlaxGPT2MLP(nn.Module):
    ...
    def setup(self):
        ...
        if USE_LORA:
            self.c_fc_A = FlaxConv1D(RANK, dtype=self.dtype, use_bias=False)
            self.c_fc_B = FlaxConv1D(self.intermediate_size, dtype=self.dtype, use_bias=False)
            self.c_proj_A = FlaxConv1D(RANK, dtype=self.dtype, use_bias=False)
            self.c_proj_B = FlaxConv1D(embed_dim, dtype=self.dtype, use_bias=False)
        else:
    ...
    def __call__(self, hidden_states, deterministic: bool = True):
        if USE_LORA:
            hidden_states_A = self.c_fc_A(hidden_states)
            hidden_states = self.c_fc_B(hidden_states_A)
        else:
            hidden_states = self.c_fc(hidden_states)
        hidden_states = self.act(hidden_states)
        if USE_LORA:
            hidden_states_A = self.c_proj_A(hidden_states)
            hidden_states = self.c_proj_B(hidden_states_A)
        else:
...
class FlaxGPT2LMHeadModule(nn.Module):
    ...
    def __call__(
        ...
        if self.config.tie_word_embeddings:
            ...
            if USE_LORA:
                lm_logits_A = self.lm_head_A(hidden_states)
                lm_logits = self.lm_head_B(lm_logits_A)
            else:
